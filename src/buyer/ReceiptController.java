...
public class ReceiptController extends PurchaseController {
    ...
    public void reducestocks(){
        ...
        try{
            ...
            ResultSet order = sta.executeQuery(sql);
            while(order.next()){
                stall = order.getString("stall");
                System.out.println(stall);
                try {
                    Connection conn2 = DriverManager.getConnection("jdbc:postgresql://localhost:5432/receipts", "postgres", "OnlinePalengke123");
                    Statement sta2 = conn2.createStatement();
                    String sql2 = "SELECT * from public."+getName()+"_receipt";
                    ResultSet order2 = sta2.executeQuery(sql2);
                    while(order2.next()){
                        if(order2.getString("stall").equals(order.getString("stall_name"))){
                            try{
                                Connection conn3 = DriverManager.getConnection("jdbc:postgresql://localhost:5432/product","postgres","OnlinePalengke123");
                                Statement sta3 = conn3.createStatement();
                                String sql3 = "select * from public."+stall;
                                ResultSet order3 = sta3.executeQuery(sql3);
                                while(order3.next()){
                                    if(order2.getString("product").toLowerCase().equals(order3.getString("product"))){
                                        splitqu = order2.getString("quantity").split("\\s")[0];
                                        splitqu = splitqu.split("\\.")[0];
                                        newstocks = order3.getInt("stock")-Integer.parseInt(splitqu);
                                        System.out.println(newstocks);
                                        Connection conn5 = DriverManager.getConnection("jdbc:postgresql://localhost:5432/product","postgres","OnlinePalengke123");
                                        String sql5 = "UPDATE " +stall+ " set stock = ? where product = ?";
                                        PreparedStatement preparedStatement = conn5.prepareStatement(sql5);
                                        preparedStatement.setInt(1, newstocks);
                                        preparedStatement.setString(2, order3.getString("product"));
                                        preparedStatement.executeUpdate();
                                        Connection conn6 = DriverManager.getConnection("jdbc:postgresql://localhost:5432/product","postgres","OnlinePalengke123");
                                        String sql6 = "INSERT INTO  " +stall+ "_orderhistory (buyer,product,quantity,totalprice) VALUES (?,?,?,?)";
                                        PreparedStatement preparedStatement2 = conn6.prepareStatement(sql6);
                                        preparedStatement2.setString(1, getName());
                                        preparedStatement2.setString(2, order3.getString("product"));
                                        preparedStatement2.setString(3, order2.getString("quantity"));
                                        preparedStatement2.setInt(4, order2.getInt("totalprice"));
                                        preparedStatement2.executeUpdate();
                                    }
                                }
                            }catch (SQLException e){
